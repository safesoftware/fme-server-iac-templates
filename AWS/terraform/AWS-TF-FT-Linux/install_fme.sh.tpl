...
file = /mnt/efs/fs1/index.cfg
if [ ! -f "$file" ]; then
    printf "0" > index.cfg
    cp /home/ubuntu/loadbalance.py /mnt/efs/fs1/loadbalance.py
fi
...
if [ ! -f "$file" ]; then
    ...
    export PGPASSWORD='postgres'; psql -U postgres -d postgres -h ${rdsAddress} -p ${rdsPort} -c "ALTER SYSTEM SET max_connections TO ‘200’;" &> changeConnections.txt
fi
