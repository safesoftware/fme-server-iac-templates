...
if ($connectTestResult.TcpTestSucceeded) {
    ...
    # Mount the drive
    New-SmbGlobalMapping -RemotePath "\\$storageAccountPath" -Credential $cred -LocalPath Z: -FullAccess @("NT AUTHORITY\SYSTEM", "NT AUTHORITY\NetworkService", "Administrator") -Persistent $True
...
$schemaExists = & "C:\Program Files\FMEServer\Utilities\pgsql\bin\psql.exe" -h ${databasehostname} -d fmeserver -p 5432 -c "SELECT EXISTS(SELECT * FROM information_schema.tables WHERE table_name = 'fme_config_props')" -w -t -U $fmeDatabaseUsername 2>&1
if($schemaExists -like "*t*") {
    ...
    $env:PGPASSWORD = "fmeserver"
    & "C:\Program Files\FMEServer\Utilities\pgsql\bin\psql.exe" -d fmeserver -h $databasehostname -U $fmeDatabaseUsername -p 5432 -f "C:\Program Files\FMEServer\Server\database\postgresql\postgresql_createSchema.sql" >"C:\Program Files\FMEServer\resources\logs\installation\CreateSchema.log" 2>&1
}
