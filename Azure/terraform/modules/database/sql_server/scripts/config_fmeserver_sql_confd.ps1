...
$schemaExists = Invoke-Sqlcmd -Query "SELECT CASE WHEN COUNT(*) >= 1 THEN CAST(1 as BIT) ELSE CAST(0 as BIT) END FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'fme_config_props'" -ServerInstance $databasehostname -Database fmeserver -Username $adminUsername -Password $adminPassword
if($schemaExists[0][0] -like "*True*") {
    Write-Host "The schema already exists"
}
else {
    #Create fmeserver user in master DB
    ...
    "
    Invoke-Sqlcmd -Query $query_2 -ServerInstance $databasehostname -Database fmeserver -Username $adminUsername -Password $adminPassword
    #Run sqlserver_createDB.sql script
    Invoke-Sqlcmd -InputFile "C:\Program Files\FMEServer\Server\database\sqlserver\sqlserver_createDB.sql" -ServerInstance $databasehostname -Database fmeserver -Username $databaseUsername -Password $databasePassword | Out-File -FilePath "C:\Program Files\FMEServer\resources\logs\installation\CreateSQLServerDatabase.log"   
   } 
